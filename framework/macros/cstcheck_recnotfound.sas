...
%macro cstcheck_recnotfound(_cstControl=)
  ...
  %do;
    ...
    %do _keys=1 %to %SYSFUNC(countw(&_cstDSKeys1,' '));
      %if %upcase(%SYSFUNC(kscan(&_cstDSKeys1,&_keys,' '))) = %upcase(%SYSFUNC(kscan(&_cstDSKeys2,&_keys,' '))) %then
      %do;
        %if %length(&_cstColumnStr)=0 %then
          %let _cstColumnStr = %SYSFUNC(kscan(&_cstDSKeys1,&_keys,' '));
        %else
          %let _cstColumnStr = &_cstColumnStr %SYSFUNC(kscan(&_cstDSKeys1,&_keys,' '));
      %end;
      %else
      %do;
        %* Stop processing to avoid later non-sequential common columns *;
        %let _keys=%SYSFUNC(countw(&_cstDSKeys1,' '));
      %end;
    %end;
  %end;
...
%exit_error:
  ...
  %end;
  %else
    %put <<< cstcheck_recnotfound;

%mend cstcheck_recnotfound;
