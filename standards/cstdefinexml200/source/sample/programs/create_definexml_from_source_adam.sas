...
*                                                                                *;
* create_definexml_from_source_adam.sas                                          *;
*                                                                                *;
* Sample driver program to perform a primary Toolkit action, in this case,       *;
* creating the Define-XML data sets from the CDISC ADaM standard model           *;
* source metadata) in the WORK library, and then create a CDISC-DEFINE-XML       *;
...
proc sql;
  ...
  values ("&_cstStandard"     "&_cstStandardVersion"    "results"         "results"    "results"  "libref"  "output" "dataset"  "Y"  "" "&studyOutputPath/results"  .  "sourcetodefinexml_adam_results.sas7bdat"     "")
