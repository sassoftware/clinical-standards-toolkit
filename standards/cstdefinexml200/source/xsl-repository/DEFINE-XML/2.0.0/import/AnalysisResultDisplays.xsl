<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:odm="http://www.cdisc.org/ns/odm/v1.3"
	xmlns:def="http://www.cdisc.org/ns/def/v2.0"
	xmlns:arm="http://www.cdisc.org/ns/arm/v1.0">

  <xsl:import href="AnalysisResults.xsl" />
  
  <xsl:template name="AnalysisResultDisplays">	
    
	  <xsl:for-each select="arm:AnalysisResultDisplays/arm:ResultDisplay">

	    <xsl:element name="AnalysisResultDisplays">
         <xsl:element name="OID"><xsl:value-of select="@OID"/></xsl:element>
         <xsl:element name="Name"><xsl:value-of select="@Name"/></xsl:element> 
         <xsl:element name="FK_MetaDataVersion"><xsl:value-of select="../../@OID"/></xsl:element>
      </xsl:element>
      
    </xsl:for-each>
    
	  <xsl:for-each select="arm:AnalysisResultDisplays/arm:ResultDisplay/arm:AnalysisResult">
	    <xsl:call-template name="AnalysisResults"/>
	  </xsl:for-each>

	</xsl:template>
</xsl:stylesheet>