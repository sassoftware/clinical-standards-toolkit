...
%macro define_sourcedocuments(
    ...
    data _cst_doc_tmp0_&_cstRandom;
      set _cstSourceDocuments2_&_cstRandom(keep=studyversion doctype doctype2 href title);
      ...
      if PDFPageRefType="PhysicalRef" then do;
        if index(PDFPageRefs, "-") then do;
          ...
          if missing(FirstPage) or kverify(kstrip(kscan(PDFPageRefs, 1, "-")),'0123456789') 
            then do;
              put "WAR%str(NING): [CSTLOG%str(MESSAGE).&sysmacroname] Invalid FirstPage in " DocType= table= column= whereclause= parent= parentKey= PDFPageRefs=;
              FirstPage=.;
          ...
          if missing(LastPage) or kverify(kstrip(kscan(PDFPageRefs, 2, "-")),'0123456789')
            then do;
              put "WAR%str(NING): [CSTLOG%str(MESSAGE).&sysmacroname] Invalid LastPage in " DocType= table= column= whereclause= parent= parentKey= PDFPageRefs=;
              LastPage=.;
          ...
          if (not missing(FirstPage)) and (not missing(LastPage)) 
           ...
           if compress(pdfpagerefs, , 'kds') ne pdfpagerefs 
             then do;
               put "WAR%str(NING): [CSTLOG%str(MESSAGE).&sysmacroname] Invalid physical PDFPageRefs in " DocType= table= column= whereclause= parent= parentKey= PDFPageRefs=;
              message = "Invalid physical PDFPageRefs";
             end;
        end;  
      end;  
      if (missing(parent) or missing(parentKey)) and doctype ne "SUPPDOC" then do;
        put "WAR%str(NING): [CSTLOG%str(MESSAGE).&sysmacroname] Document is not referenced and will be ignored: " DocType= href= title= table= column= whereclause= parent= parentKey=;
    ...
    data work._cstIssues_&_cstRandom(drop=href title DocType PDFPageRefType PDFPageRefs table column whereclause);
      set work._cstIssues_&_cstRandom
         work._cst_doc_tmp2_&_cstRandom(where=(not missing(message)) keep=message href title DocType PDFPageRefType PDFPageRefs table column whereclause);
      resultid="DEF0098";
      ...
      resultflag=1;
      _cst_rc=0;
      message=catt(message, ": DocType=", DocType, ", href=", href, ", Title=", title);
      if not missing(PDFPageRefs) then message=catt(message, ", PDFPageRefType=", PDFPageRefType, ", PDFPageRefs=", PDFPageRefs);
      message=catt(message, ", Table=", table, ", Column=", column, ", WhereClause=", whereclause);
    run;
