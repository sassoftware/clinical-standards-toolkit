...
%*                                                                                *;
%* Derives source study metadata files from a Define-XML v2.0 data library.       *;
%*                                                                                *;
%* This macro derives source metadata files from a data library that contains     *;
%* the SAS representation of a Define-XML V2.0.0 define.xml file for a study.     *;
%*                                                                                *;
%* These SAS data sets must exist in this Define-XML V2.0.0 SAS data set library: *;
%*      aliases                                                                   *;
%*      codelistitems                                                             *;
%*      codelists                                                                 *;
%*      definedocument                                                            *;
...
%* must exist in this Define-XML V2.0.0 SAS data set library:                     *;
%*                                                                                *;
%*      analysisdataset                                                           *;
%*      analysisdatasets                                                          *;
...
%*    1. Use the SAS representation of a Define-XML V2.0.0 define.xml file as the *;
...
%*   3. _cstTrg<table>DS must be specified (table=Study|Table|Column|CodeList|    *;
%*      Value|Document|AnalysisResults). If this parameter is not specified, the  *;
%*      macro attempts to get _cstSAS<table>Lib from the SASReferences data set   *;
%*      that is specified by the macro variable _cstSASRefs (type=studymetadata,  *;
...
%*             representation of a Define-XML V2.0.0 file is located.             *;
...
%macro define_createsrcmetafromdefine(
  ...
  %let _cstExpTables=definedocument study metadataversion supplementaldocs itemgroupdefs itemgroupitemrefs itemdefs translatedtext %str
                     ()itemgroupleaf itemgroupleaftitles aliases codelists itemorigin methoddefs formalexpressions externalcodelists %str
                     ()codelistitems enumerateditems whereclausedefs whereclauserangechecks whereclauserangecheckvalues %str
                     ()valuelists valuelistitemrefs itemvaluelistrefs itemrefwhereclauserefs mdvleaf mdvleaftitles documentrefs pdfpagerefs;
  %let _cstMissing=;
  ...
  %let _cstExpTables=Study Table Column CodeList Value Document;
  ...
  proc sql;
    ...
    as select
      ...
      mdv.StandardName as formalstandardname,
      mdv.StandardVersion as formalstandardversion,
  ...
  %do;
    ...
    %cstutil_deleteDataSet(_cstDataSetName=work.source_tables_tmplt_&_cstRandom);
  ...
  proc sql;
    ...
    as select
     ...
     on extcl.fk_CodeLists = cl.oid
       ...
       (select cli.*, clitt.lang, clitt.TranslatedText, clial.Name from
       ...
       (select encli.*, enclial.Name from
  ...
  proc sql;
    create table work.source_suppdocuments_&_cstRandom
    as select
    ...
    create table work.source_documents0_&_cstRandom
    as select
    ...
    create table work.source_documents_&_cstRandom(drop=parent parentkey originitem)
    as select * from
    (select
      sd0.*,
    ...
    from work.source_documents0_&_cstRandom sd0
  ...
  %let _cstRecs=%cstutilnobs(_cstDatasetName=work.source_documents_&_cstRandom);
  %if %eval(&_cstRecs) gt 0 %then %do;

    data &_cstTrgDocumentDS;
      set &_cstTrgDocumentDS 
          work.source_suppdocuments_&_cstRandom
          work.source_documents_&_cstRandom;
    run;
  ...
  %cstutil_deleteDataSet(_cstDataSetName= work.source_suppdocuments_&_cstRandom);
  %cstutil_deleteDataSet(_cstDataSetName=work.source_documents0_&_cstRandom);
  %cstutil_deleteDataSet(_cstDataSetName=work.source_documents_&_cstRandom);
  ...
  %if &_cstAnalysisResultsMetadata %then %do;
    ...
    as select 
     ...
     from work.source_displays_&_cstRandom ad
       inner join work.source_displayresults_&_cstRandom(drop=sasref StudyVersion Standard Standard StandardVersion) adr
     on adr.FK_AnalysisResultDisplays = ad.DisplayIdentifier
       inner join work.source_displayresultds_&_cstRandom(drop=sasref StudyVersion Standard Standard StandardVersion) adrd
