...
%*   3. _cstStudyMetadata references a data set with Study metadata in these      *;
%*      columns:                                                                  *;
...
%*   4. _cstTrg<table>DS must be specified (table=Study|Table|Column|CodeList|    *;
%*      Value|Document). If this parameter is not specified, the macro attempts   *;
%*      to get _cstSAS<table>DS from the SASReferences data set that is           *;
...
%*      filetype=dataset).                                                        *;
%*   5. _cstUseRefLib=Y _cstRefTableDS must be specified. If this parameter is    *;
...
%*   6. _cstUseRefLib=Y _cstRefColumnDS must be specified. If this parameter is   *;
...
%*      filetype=dataset).                                                        *;
%*   7. _cstUseRefLib=Y _cstClassTableDS must be specified. If this parameter is  *;
...
%*      filetype=dataset).                                                        *;
%*   8. _cstUseRefLib=Y _cstClassColumnDS must be specified. If this parameter is *;
...
%macro define_createsrcmetafromsaslib(
  ...
  * A single source data library serves as the input to this process.  *;
  ...
  proc sql noprint;
   ...
   where upcase(standard) = "%upcase(&_cstTrgStandard)"
   ;
  quit;
  ...
  data &_cstTrgStudyDS
    ...
    set &_cstTrgStudyDS 
        &_cstStudyMetadata(keep=studyname studydescription protocolname studyversion);
    sasref= upcase("&_cstSASDataLib");
    standard = "&_cstTrgStandard";
    StandardVersion= "&_cstTrgStandardVersion";
    formalstandardname="&_styFormalStandardName";
    formalstandardversion="&_cstTrgStandardVersion";
  run;
  ...
  %do;
    ...
    proc sort data=work.contents&_cstRandom 
      out=work.source_tables_&_cstRandom
      (keep=memname memlabel name sorted sortedby rename=(memname=table memlabel=label));
    by memname sortedby;
    run;   
    ...
    data work.source_tables_&_cstRandom(drop=name sorted sortedby);
      ...
      by table sortedby;
      length xmltitle xmlpath keys $400 domain class $32;
    ...
    %cstutiltrimcharvars(
      _cstDataSetName=work.source_tables_&_cstRandom,
      _cstDataSetOutName=work.source_tables_&_cstRandom
    );
  ...
  %do;
    ...
    proc sort data=work.contents&_cstRandom 
      out=work.source_columns_&_cstRandom
      (drop=libname memlabel sorted sortedby 
       rename=(memname=table name=column type=ctype varnum=order));
      by memname varnum;
    run;   
  ...
  %let _cstDSLabel=%cstutilgetattribute(_cstDataSetName=work.source_codelists_&_cstRandom,_cstAttribute=LABEL);
