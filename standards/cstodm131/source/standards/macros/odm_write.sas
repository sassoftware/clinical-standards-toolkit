...
%*            Values: 0 | 1                                                       *;
%*            Default: 0                                                          *;
%* @param _cstOutputEncoding - optional - The XML encoding to use for the ODM     *;
...
%macro odm_write(
    _cstCreateDisplayStyleSheet=0,
    _cstOutputEncoding=UTF-8,
    _cstHeaderComment=,
    _cstResultsOverrideDS=,
    _cstLogLevel=info
    ) / des='CST: Write ODM XML file';
  ...
  %odm_getStatic(_cstName=ODM_SASREF_SUBTYPE_STYLESHEET,_cstVar=_cstSubtypeStylesheet);

  %odm_getStatic(_cstName=ODM_JAVA_PARAMSCLASS,_cstVar=_cstParamsClass);
  %odm_getStatic(_cstName=ODM_JAVA_EXPORTCLASS,_cstVar=_cstTransformsClass);
  ...
  * The sasrefs data set must exist;
  ...
  %*************************************************************;
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS STANDARD: &_cstrunstd,_cstSeqNoParm=1,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS STANDARDVERSION: &_cstrunstdver,_cstSeqNoParm=2,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS DRIVER: CREATE_ODMXML,_cstSeqNoParm=3,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS DATE: &_cstrundt,_cstSeqNoParm=4,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS TYPE: CREATE ODM XML ,_cstSeqNoParm=5,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS SASREFERENCES: &_cstrunsasref,_cstSeqNoParm=6,_cstSrcDataParm=ODM_WRITE);
  %if %symexist(studyRootPath) %then
    %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS STUDYROOTPATH: &studyRootPath,_cstSeqNoParm=7,_cstSrcDataParm=ODM_WRITE);
  %else
    %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS STUDYROOTPATH: <not used>,_cstSeqNoParm=7,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS GLOBALLIBRARY: &_cstGRoot,_cstSeqNoParm=8,_cstSrcDataParm=ODM_WRITE);
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS CSTVERSION: &_cstVersion,_cstSeqNoParm=9,_cstSrcDataParm=ODM_WRITE);
  %let _cstSeqCnt=9;
  ...
  * _cstCreateDisplayStyleSheet must be 0 or 1;
  ...
  %if (&_cstCreateDisplayStyleSheet=1) %then %do;
    ...
    %if (&_cstDebug) %then %do;
      %put _cstTempFilename5: &_cstTempFilename5;
      %put PATH: %sysfunc(pathname(&_cstTempFilename5));
    %end;
  ...
  %* This next section ensures that the dataset and column names have the correct
     ...
     These will be used in the ODM XML creation and then deleted later.;
  ...
  * Note: there may be issues if this is buried in multiple macro calls;
   ...
   * The source data sets must exist;
   %if (&_cstTempRefMdTableObs = 0) %then %do;
      %let _cstParam1=No SAS data sets to transform to ODM in library &_cstSrcDataLibrary..;
      %let _cstParam2=;
...
* De-Allocate the temporary filename;
  ...
  %cstutil_writeODMcubexml(_cstXMLOut=&_cstCubeXMLPath, _cstEncoding=&_cstOutputEncoding);
  ...
  * Cleanup the log and cube file;
  ...
  %cstutil_writeresult(
                _cstResultId=ODM0010
                ,_cstResultParm1=&_cstParam1
                ,_cstResultSeqParm=&_cstResultSeq
                ,_cstSeqNoParm=&_cstSeqCnt
                ,_cstSrcDataParm=ODM_WRITE
