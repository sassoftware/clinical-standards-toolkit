...
%*                                                                                *;
%* This macro derives source metadata files from a CRT-DDS data library that is   *;
%* derived from the define.xml file for a CDISC ADAM study.                       *;
%*                                                                                *;
...
%* @macvar _cstReflib ADAM reference library                                      *;
%* @macvar _cstRefTableDS ADAM reference table data set                           *;
%* @macvar _cstRefColumnDS ADAM reference column data set                         *;
%* @macvar _cstADAMDataLib ADAM metadata library                                  *;
...
%macro adamutil_createsrcmetafromcrtdds(
    ) / des='CST: Create ADAM metadata from CRTDDS Data';
  ...
  proc sql noprint;
    ...
    select ds14.*, ds23.* from
      (select ds1.oid,
              upcase("&_cstADAMDataLib") as sasref1,
              case
  ...
  * Clear the filename;
  ...
  proc sql;
    ...
    select
      upcase("&_cstADAMDataLib") as sasref 'SAS metadata libref',
...
%mend adamutil_createsrcmetafromcrtdds;