...
%* @macvar _cstSENDDataLib SEND Source data library                               *;
...
%*                                                                                *;
%* @history 2013-11-17 Removed comment content because this is not the            *;
...
%macro sendutil_createsrcmetafromsaslib(
    ) / des='CST: Create SEND metadata from SAS library';
...
proc sql noprint;
  ...
  select upcase("&_cstSENDDataLib") as sasref length=8 format=$8. label="SASreferences sourcedata libref",
         ref.table,
...
%mend sendutil_createsrcmetafromsaslib;