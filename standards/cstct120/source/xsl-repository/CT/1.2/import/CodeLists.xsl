<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="1.0"
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:odm="http://www.cdisc.org/ns/odm/v1.3"
  xmlns:nciodm="http://ncicb.nci.nih.gov/xml/odm/EVS/CDISC">

  <xsl:template name="CodeLists">

    <xsl:for-each select="odm:CodeList">

      <xsl:element name="CodeLists">
         <xsl:element name="OID"><xsl:value-of select="@OID"/></xsl:element>
         <xsl:element name="Name"><xsl:value-of select="@Name"/></xsl:element>
         <xsl:element name="DataType"><xsl:value-of select="@DataType"/></xsl:element>
         <xsl:element name="SASFormatName"><xsl:value-of select="@SASFormatName"/></xsl:element>
         <xsl:element name="ExtCodeID"><xsl:value-of select="@nciodm:ExtCodeID"/></xsl:element>
         <xsl:element name="CodeListExtensible"><xsl:value-of select="@nciodm:CodeListExtensible"/></xsl:element>
         <xsl:if test="string-length(normalize-space(nciodm:CDISCSubmissionValue)) &gt; 0">
             <xsl:element name="CDISCSubmissionValue"><xsl:value-of select="nciodm:CDISCSubmissionValue/text()"/></xsl:element>
         </xsl:if>
         <xsl:if test="string-length(normalize-space(nciodm:PreferredTerm)) &gt; 0">
             <xsl:element name="PreferredTerm"><xsl:value-of select="nciodm:PreferredTerm/text()"/></xsl:element>
         </xsl:if>
         <xsl:element name="FK_MetaDataVersion"><xsl:value-of select="../@OID"/></xsl:element>
      </xsl:element>

    </xsl:for-each>

  </xsl:template>
</xsl:stylesheet>