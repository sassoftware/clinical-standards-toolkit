...
%* ct_read                                                                        *;
%*                                                                                *;
%* Reads a CDISC-CT V1.2.0 XML file into the SAS representation of CT 1.2.0.      *;
%*                                                                                *;
...
%macro ct_read(
    ...
    ) / des='CST: Read CT V1.0.0 XML file';
  ...
  %ct_getStatic(_cstName=CT_JAVA_IMPORTCLASS,_cstVar=_cstTransformsClass);
  ...
  * The sasrefs data set must exist;
  ...
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS DRIVER: CREATE_SASCT_FROMXML,_cstSeqNoParm=3,_cstSrcDataParm=&_cstSrcData);
...
* De-Allocate the temporary filename;
  ...
  * check to see if there are any Java issues;
  ...
  %do;
    ...
    %*************************************************************************************;
    %* Generate dynamic XML map file for CT using Reference Columns and Tables Metadata  *;
    %*************************************************************************************;
    ...
    data _null_;
      ...
      do;
        ...
        put '<SXLEMAP name="CT100" version="1.2">';
        put ;
      end;
