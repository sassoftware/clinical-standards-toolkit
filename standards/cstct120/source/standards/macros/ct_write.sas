...
%* ct_write                                                                       *;
%*                                                                                *;
%* Writes an CDISC-CT V1.2.0 XML file.                                            *;
%*                                                                                *;
...
%*            Values: 0 | 1                                                       *;
%*            Default: 0                                                          *;
%* @param _cstOutputEncoding - optional - The XML encoding to use for the CT      *;
...
%macro ct_write(
    ...
    _cstLogLevel=info
    ) / des='CST: Write CT 1.2.0 XML file';
  ...
  %ct_getStatic(_cstName=CT_SASREF_TYPE_REFXML,_cstVar=_cstTypeRefXML);
  %ct_getStatic(_cstName=CT_SASREF_TYPE_EXTXML,_cstVar=_cstTypeExtXML);
  %ct_getStatic(_cstName=CT_SASREF_SUBTYPE_XML,_cstVar=_cstSubtypeXML);
  %ct_getStatic(_cstName=CT_SASREF_SUBTYPE_STYLESHEET,_cstVar=_cstSubtypeStylesheet);

  %ct_getStatic(_cstName=CT_JAVA_PARAMSCLASS,_cstVar=_cstParamsClass);
  %ct_getStatic(_cstName=CT_JAVA_EXPORTCLASS,_cstVar=_cstTransformsClass);
  ...
  * The sasrefs data set must exist;
  ...
  data _null_;
    set &_cstSASrefs (where=(upcase(standard)="CDISC-CT"));
  ...
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS DRIVER: CREATE_CT_XML,_cstSeqNoParm=3,_cstSrcDataParm=&_cstSrcData);
  ...
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS TYPE: CREATE_CT_XML ,_cstSeqNoParm=5,_cstSrcDataParm=&_cstSrcData);
  ...
  * _cstCreateDisplayStyleSheet must be 0 or 1;
  ...
  %* This next section ensures that the dataset and column names have the correct
     ...
     These will be used in the CT XML creation and then deleted later.;
  ...
  * Note: there may be issues if this is buried in multiple macro calls;
   ...
   * The source data sets must exist;
   %if (&_cstTempRefMdTableObs = 0) %then %do;
      %let _cstParam1=No SAS data sets to transform to CT XML in library &_cstSrcDataLibrary..;
      %let _cstParam2=;
