...
* to validate the metadata defined for one or more standards.  This is metadata  *;
* that resides in the Global Library, in the Sample Library, and/or user-defined *;
* Study Libraries.                                                               *;
*                                                                                *;
* Assumptions:                                                                   *;
*   (1) This driver excludes those validation checks that are common across      *;
*        standards, i.e., those focusing primarily on the cstGlobalLibrary       *;
...
*                                                                                   *;
* The validation_control_std view of the validation_master data set includes just   *;
* those checks specific to one or more standards and excludes those core framework  *;
* checks that look only within the <cstGlobalLibrary>/metadata folder.              *;
*************************************************************************************;
...
/*
proc sql;
  create view cstcntl.validation_control_std (label="Standard-specific checks")
    as select * 
...
data _null_;
  ...
  do;
    put '%cstutil_writeresult(_cstResultID=CST0200,
          ...
          _cstSeqNoParm=0,
          _cstSrcDataParm=VALIDATE_STANDARD);';  
  end;
  else
  do;
    ...
    put '%cstutilbuildstdvalidationcode(_cstStdDS=work._cstStandardsforIV,_cstSampleRootPath=_DEFAULT_,
      _cstSampleSASRefDSPath=_DEFAULT_,_cstSampleSASRefDSName=_DEFAULT_,_cstCallingDriver=validate_standard.sas);';
    put '%include incCode;';
