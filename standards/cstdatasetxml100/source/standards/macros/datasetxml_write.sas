...
%*            Default: _cst_rcmsg                                                 *;
%*                                                                                *;
%* @history 2016-02-01 Added nrbquote to fix unbalanced quotes (DE6033).          *;
%*                                                                                *;
%* @since  1.7                                                                    *;
%* @exposure external                                                             *;
...
%macro datasetxml_write(
  ...
  %else %do;
    ...
    *** Reading Define-XML files;    
    ...
    %put NOTE: [CSTLOG%str(MESSAGE).&sysmacroname] Metadata used from %nrbquote(%sysfunc(pathname(&_cstSourceMetadatasasref)));
    %if %symexist(_cstResultsDS) %then %if %sysfunc(exist(&_cstResultsDS)) %then
    %do;
      %let _cstSeqCnt=%eval(&_cstSeqCnt+1);
      %cstutil_writeresult(
                  _cstResultId=DATA0097
                  ,_cstResultParm1=Metadata used from %nrbquote(%sysfunc(pathname(&_cstSourceMetadatasasref)))
  ...
  * Combine Metadata;
  ...
  %do _cstCounter=1 %to %sysfunc(countw(&_cstSourceTableList, %str( )));
    ...
    %let _cstDatasetXMLFile=%nrbquote(%sysfunc(pathname(&_cstOutputLibrary)))&_cstPathDelim.%lowcase(&_cstTable).xml;
    ...
    *** Create Dataset-XML file;
    ...
    %do;
      ...
      %let _cstDatasetZIPFile=%nrbquote(%sysfunc(pathname(&_cstOutputLibrary)))&_cstPathDelim.%lowcase(&_cstTable).zip;
