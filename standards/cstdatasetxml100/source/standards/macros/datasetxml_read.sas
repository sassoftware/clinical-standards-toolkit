...
%* @history 2016-02-01 Added file:/// protocol to javaobj method call.            *;
...
%macro datasetxml_read(
  ...
  %if %kverify(&_cstMaxLabelLength,0123456789)>0 %then
  %do;
  ...
  %if %kverify(&_cstdatetimeLength,0123456789)>0 %then
  %do;
  ...
  %do;
    %if not %sysfunc(fileexist(%nrbquote(&_cstSourceDatasetXMLFile))) %then
    %do;
      %* Rule: If _cstSourceDatasetXMLFile is specified, it must exist  *;
      %let &_cstReturn=1;
      %let &_cstReturnMsg=The file _cstSourceDatasetXMLFile=%nrbquote(&_cstSourceDatasetXMLFile) does not exist.;
      %goto exit_error;
    %end;
  %end;
  ...
  %do;
    ...
    %let _cstMetadataSource=%nrbquote(%sysfunc(pathname(&_cstSourceMetadatasasref)));
  ...
  %put NOTE: [CSTLOG%str(MESSAGE).&sysmacroname] Metadata used from %nrbquote(&_cstMetadataSource);
  %if %symexist(_cstResultsDS) %then %if %sysfunc(exist(&_cstResultsDS)) %then
  %do;
    %let _cstSeqCnt=%eval(&_cstSeqCnt+1);
    %cstutil_writeresult(
                _cstResultId=DATA0097
                ,_cstResultParm1=Metadata used from %nrbquote(&_cstMetadataSource)
  ...
  *** Combine Metadata;
  ...
  %do;
    data DatasetXMLfiles_&_cstRandom;
      ...
      xmlfilepath="%nrbquote(&_cstSourceDatasetXMLFile)";
  ...
  %do _cstCounter=1 %to &_cstDatasetXMLFiles;
    ...
    *** Combining ReferenceData and ClinicalData;
    ...
    %put NOTE: [CSTLOG%str(MESSAGE).&sysmacroname] Data read from %nrbquote(&_cstDatasetXMLFile);
    %if %symexist(_cstResultsDS) %then %if %sysfunc(exist(&_cstResultsDS)) %then
    %do;
      %let _cstSeqCnt=%eval(&_cstSeqCnt+1);
      %cstutil_writeresult(
                  _cstResultId=DATA0097
                  ,_cstResultParm1=Data read from %nrbquote(&_cstDatasetXMLFile)
                  ,_cstResultSeqParm=&_cstResultSeq
                  ,_cstSeqNoParm=&_cstSeqCnt
                  ,_cstSrcDataParm=&_cstSrcMacro
                  ,_cstResultFlagParm=&_cst_rc
    ...
    %if &_cstNobs eq 0 %then %do;
      %put WAR%str(NING): [CSTLOG%str(MESSAGE).&sysmacroname] No data found in %nrbquote(&_cstDatasetXMLFile);
      %if %symexist(_cstResultsDS) %then %if %sysfunc(exist(&_cstResultsDS)) %then
      %do;
        %let _cstSeqCnt=%eval(&_cstSeqCnt+1);
        %cstutil_writeresult(
                    _cstResultId=DATA0098
                    ,_cstResultParm1=No data found in %nrbquote(&_cstDatasetXMLFile)
                    ,_cstResultSeqParm=&_cstResultSeq
                    ,_cstSeqNoParm=&_cstSeqCnt
                    ,_cstSrcDataParm=&_cstSrcMacro
                    ,_cstResultFlagParm=0
