...
%*                                                                                *;
%* Creates the SAS CRT-DDS ItemGroupLeafTitles data set from source metadata.     *;
%*                                                                                *;
...
%* @param _cstoutitemgroupleaftitlesds - required - The ItemGroupLeafTitles       *;
%*            data set to create (for example, srcdata.ItemGroupLeafTitles).      *;
%*                                                                                *;
%* @since 1.5                                                                     *;
%* @exposure external                                                             *;

%macro crtdds_itemgroupleaftitles(
    _cstsourcetables=,
    _cstsourcestudy=,
    _cststudyds=,
    _cstmdvDS=,
    _cstoutitemgroupleaftitlesds=
    ) / des="Creates SAS CRT-DDS ItemGroupLeafTitles data set";
...
%if %length(&_cstsourcetables)=0 or %length(&_cstsourcestudy)=0 or %length(&_cststudyds)=0
  or %length(&_cstmdvDS)=0 or %length(&_cstoutitemgroupleaftitlesds)=0
    %then %goto exit;
...
%* need to check if 2 level name passed in for parameters, pull off dataset name for use in proc sql *;
 ...
 %if %symexist(_cststudyds) %then %do;
  ...
  %if %symexist(_cstmdvDS) %then %do;
    ...
    %end;
  %end;
  %if %symexist(_cstoutitemgroupleaftitlesds) %then %do;
      %if %sysfunc(indexc("&_cstoutitemgroupleaftitlesds",'.'))=0 %then %let iglds=&_cstoutitemgroupleaftitlesds;
      %else %do;
        %let igl2=%scan(&_cstoutitemgroupleaftitlesds,2,'.');
    %let iglds=&igl2;
    %end;
  %end;
...
proc sql;
  ...
  select
   ...
   &ds5..xmltitle as title length = 2000,
...
data &_cstoutitemgroupleaftitlesds(keep=title fk_itemgroupleaf);
length FK_ItemGroupLeaf $128;
set &_cstoutitemgroupleaftitlesds &ds4;
FK_ItemGroupLeaf="Location."||kstrip(kcompress(table));
RUN;
...
%mend crtdds_itemgroupleaftitles;
