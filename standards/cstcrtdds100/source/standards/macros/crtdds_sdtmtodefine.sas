...
%*                                                                                *;
%* This macro extracts data from the SDTM metadata files (_cstSource*) and        *;
...
%* @macvar _cstTrgStandardVersion The SDTM version of interest, defined in the    *;
...
%macro crtdds_sdtmtodefine(
    ...
    _cstSourceValues=,
    _cstSourceDocuments=
    ) / des='CST: Build CRTDDS files from SDTM';
...
%cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS TYPE: SDTM TO CRTDDS ,_cstSeqNoParm=5,_cstSrcDataParm=&_cstThisMacro);
...
%if %sysfunc(exist(&_cstOutlib..itemgroupdefs)) %then %do;
   %cstutil_writeresult(
                ...
                ,_cstResultSeqParm=1
                ,_cstSeqNoParm=1
                ,_cstSrcDataParm=&_cstThisMacro..CRTDDS_ITEMGROUPDEFS
                ,_cstResultFlagParm=0
                ,_cstRCParm=0
                ,_cstResultsDSParm=&_cstResultsDS
                );
...
%if %sysfunc(exist(&_cstOutlib..itemgroupdefitemrefs)) %then %do;
   %cstutil_writeresult(
                ...
                ,_cstResultSeqParm=1
                ,_cstSeqNoParm=1
                ,_cstSrcDataParm=&_cstThisMacro..CRTDDS_ITEMGROUPDEFITEMREFS
                ,_cstResultFlagParm=0
                ,_cstRCParm=0
                ,_cstResultsDSParm=&_cstResultsDS
                );
