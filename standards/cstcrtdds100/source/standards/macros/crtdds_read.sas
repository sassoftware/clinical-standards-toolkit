...
%* crtdds_read                                                                    *;
%*                                                                                *;
%* Reads a CDISC CRT-DDS 1.0 XML file into the SAS representation of CRT-DDS.     *;
%*                                                                                *;
%* This macro uses the SAS representation of a CDISC CRT-DDS XML file as source   *;
...
%macro crtdds_read(
    ) / des='CST: Read CRTDDS V1.0 XML file';
  ...
  %crtdds_getStatic(_cstName=CRTDDS_JAVA_IMPORTCLASS,_cstVar=_cstTransformsClass);
  ...
  * The sasrefs data set must exist;
  ...
  %cstutil_writeresult(_cstResultID=CST0200,_cstResultParm1=PROCESS DRIVER: CREATE_SASCRTDDS_FROMXML,_cstSeqNoParm=3,_cstSrcDataParm=&_cstSrcData);
...
* De-Allocate the temporary filename;
  ...
  * check to see if there are any Java issues;
  ...
  %do;
    ...
    %*************************************************************************************;
    %* Generate dynamic XML map file for CRT using Reference Columns and Tables Metadata *;
    %*************************************************************************************;
    ...
    proc sort data=&_cstRefLib..&_cstRefTableDS out=work._cst&_cstTempDS1 (rename=(label=tlabel) keep=table label);
      by table;
    run;
    ...
    data _null_;
      ...
      if first.table then
      do;
         codeline='<TABLE name="'||strip(table)||'">';
         put codeline;
         codeline='<TABLE-PATH syntax="XPath">/LIBRARY/'||strip(table)||'</TABLE-PATH>';
         put @4 codeline;
      ...
      codeline='<COLUMN name="'||strip(column)||'">';
      put @4 codeline;
      codeline='<PATH syntax="Xpath">/LIBRARY/'||strip(table)||'/'||strip(column)||'</PATH>';
      put @6 codeline;
