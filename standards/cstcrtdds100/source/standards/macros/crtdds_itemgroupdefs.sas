...
proc sql;
  ...
  select
   ...
   ifc(INDEXW(UPCASE(&ds5..keys) ,"USUBJID"),'No','Yes') as IsReferenceData length = 3,
    ...
    &ds5..table as Domain length = 2000,
...
if kstrip(xmlPath) ne '' then
  ArchiveLocationID="Location."||Strip(kcompress(Name));
